...
class DatabaseManager:
    ...
    def save_link(self, link_info: dict, keyword: str) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute('''
                    INSERT OR IGNORE INTO telegram_links (link, source, keyword)
                    VALUES (?, ?, ?)
    ...
    def link_exists(self, link: str) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT 1 FROM telegram_links WHERE link = ? LIMIT 1
            ''', (link,))
...
# ---------------- 启动浏览器让用户登录（使用Playwright统一管理） ----------------
def launch_browser_for_login(storage_state="storage_state.json"):
    """
    使用Playwright启动浏览器供用户登录
    这样可以统一管理浏览器实例，避免多个浏览器进程
    """
    try:
        print("[Worker] 启动浏览器供用户登录...", flush=True)
        with sync_playwright() as p:
            ...
            # 导航到X登录页面
            print("[Worker] 正在打开 X (Twitter) 登录页面...", flush=True)
            page.goto("https://x.com/login", wait_until="load", timeout=30000)
            
            print("[Worker] 浏览器已启动！", flush=True)
            print("[Worker] 请在浏览器中完成登录，然后点击主界面的'确认登录完成'按钮", flush=True)
            
            # 等待用户手动登录 - 保持浏览器打开
            input("[Worker] 登录完成后请按回车键继续...")
            
            # 保存登录状态
            context.storage_state(path=storage_state)
            print(f"[Worker] 登录状态已保存到 {storage_state}", flush=True)
...
# ---------------- 简化的登录状态保存 ----------------
def attach_and_save_login(storage_state="storage_state.json"):
    """
    简化的登录状态保存函数，主要用于向后兼容
    实际的登录和保存现在在 launch_browser_for_login 中统一处理
    """
    if Path(storage_state).exists():
        print(f"[Worker] 登录状态文件 {storage_state} 已存在", flush=True)
    else:
        print(f"[Worker] 未找到登录状态文件 {storage_state}，请先执行登录操作", flush=True)
            ...
            for url_index, url in enumerate(urls):
                ...
                for round_num in range(rounds):
                    ...
                    for scroll_count in range(scrolls_per_round):
                        ...
                        print(f"[Worker] 当前本轮已发现 {len(本轮_links_found)} 个链接，总共保存 {total_saved_count} 个到数据库", flush=True)
